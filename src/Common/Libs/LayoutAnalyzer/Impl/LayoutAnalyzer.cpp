...
{
   if(!fs::exists(Path)) { throw std::invalid_argument("Path " + Path.string() + " does not exist!");}
   if(fs::is_directory(Path)) { throw std::invalid_argument("File argument must be passed!");}

   LayoutReaderUniquePtr reader(nullptr, [](LayoutReader* Reader) { FreeReader(Reader);});

   reader.reset(GetReader(Path.wstring()));
   if(!reader) { throw std::runtime_error("Invalid layout file"); }
   return reader;
}
...
{
   if(!fs::is_directory(LibraryDir)) { throw std::invalid_argument("");}
